...
export const adoptOp: OpModule<AdoptArgs> = {
  name: "adopt",
  async buildRequest(args, ctx): Promise<SignRequest> {
    const skim: Single = { op: "adopt", ship: args.child };
    buildAttestationLeaf(skim, ctx.authPubkey);
    return {
      v: 1, nonce: newNonce(), patp: ctx.patpAtom, op: "adopt",
      summary: `${atomToPatp(ctx.patpAtom)} adopts ${atomToPatp(args.child)}`,
