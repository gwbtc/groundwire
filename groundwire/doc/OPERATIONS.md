...
| **a snub is undone by a later positive verdict** — it was not. `+sy-sybl`'s `%full` branch (`sys/vane/ames.hoon`) installed the point and never touched `ships.snub`; only `%fail` wrote it. Verified live 2026-08-07: a comet re-verified VALID with its snub intact. | Fixed in the kernel, `f68a547b2b`: `+sy-sybl`'s `%full` branch now calls `(sy-snub %deny %del ~[her])`, so a positive verdict lifts an existing snub. This is a **safety net, not self-healing** — the `%hear` path still drops packets from a snubbed ship, so the positive verdict has to arrive by some route that is not blocked (the block scanner learning a publication, an operator-poked `%jael-writ`, or the mesa `%page` path, which has **no snub gate at all** and is an open question in its own right). On classic ames, undoing a wrong snub is still operator-driven: `ops/gwsnub.py`, or an ames `%snub %deny %del`. |
