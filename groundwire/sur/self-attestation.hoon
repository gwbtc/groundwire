...
::    current `tip` (a bare %spawn is a valid one-link chain). `precommit` is
::    the funding tx that the spawn commit spends: the networking-key tweak
::    binds to a precommit satpoint, and link 0's input 0 must spend it.
::
+$  self-attestation
  $:  who=@p
      precommit=[=txid:ord block=@ux]  :: the funding tx the spawn commit spends
