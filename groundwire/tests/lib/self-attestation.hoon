...
::  precommit -> commit -> genesis -> %no-op -> tip chain with real secp
::  taproot commitments and a suite-C key minted with the correct tweak,
::  then drives ++run-checks directly (no RPC).
::
...
::
::  ==========================================================================
::  End-to-end fixture: precommit -> commit -> genesis -> %no-op -> tip
::  ==========================================================================
::
++  pre-id  0x1a1a.1a1a
++  cmt-id  0x2b2b.2b2b
...
::
::  commit spends the precommit output; its output commits to the spawn leaf
++  cmt-tx  (mk-tx cmt-id ~[(mk-input pre-id 0 ~)] ~[[(p2tr-spk q0) 10.000]])
::  genesis (link 0) key-path-spends the commit; commits to the %no-op leaf
++  gen-tx  (mk-tx gen-id ~[(mk-input cmt-id 0 keypath-wit)] ~[[(p2tr-spk q1) 9.500]])
::  link 1 key-path-spends the genesis; its output is the tip
++  nop-tx
  %^    mk-tx
      nop-id
    ~[(mk-input gen-id 0 keypath-wit)]
  ~[[(p2tr-spk (x-only:tr (mk-ikey 31))) 9.000]]
::
++  link0  ^-  link:sa  [gen-id 0 [ikey0 spawn-leaf] ~[spawn-rs]]
++  link1  ^-  link:sa  [nop-id 0 [ikey1 noop-leaf] ~[noop-rs]]
::
++  good-sat
  ^-  self-attestation:sa
  [who pre-id ~[link0 link1] [nop-id 0 0]]
::
++  run-good
  |=  [sat=self-attestation:sa tracked=(unit sont:ord)]
  ^-  result:sa
  (run-checks:sal sat ~[gen-tx nop-tx] cmt-tx pre-tx `%.y tracked `0 ~[`0 `0])
::
++  replay
  |=  chain=(list link:sa)
  ^-  (unit point:urb)
...
++  test-link-checks-valid
  =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
    (link-checks:sal who 0 link1 gen-tx nop-tx [0 0] `0)
  ;:  weld
...
++  test-link-checks-tampered
  ;:  weld
    ::  wrong internal key -> commitment fails
    =/  l  link1
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      (link-checks:sal who 0 l(internal-key.reveal (mk-ikey 99)) gen-tx nop-tx [0 0] `0)
    (expect !>(!(got-check-in cs 'link-0-commitment')))
    ::  two-element witness -> not key-path
    =/  t  nop-tx
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      %:  link-checks:sal  who  0  link1  gen-tx
          t(is ~[(mk-input gen-id 0 ~[[64 0x0] [33 0x0]])])
          [0 0]  `0
      ==
    ...
    =/  l  link1
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      (link-checks:sal who 0 l(sots ~[(mk-rs who [%fief ~])]) gen-tx nop-tx [0 0] `0)
    (expect !>(!(got-check-in cs 'link-0-sots-match')))
    ::  empty sots -> the no-gaps rule fails
    =/  l  link1
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      (link-checks:sal who 0 l(sots ~) gen-tx nop-tx [0 0] `0)
    (expect !>(!(got-check-in cs 'link-0-sots-nonempty')))
    ::  foreign-ship sotx -> not self-enacted
    =/  l  link1
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      (link-checks:sal who 0 l(sots ~[(mk-rs ~marzod noop-sg)]) gen-tx nop-tx [0 0] `0)
    (expect !>(!(got-check-in cs 'link-0-sots-ship')))
    ::  spending an output other than the sat-carrying one -> fork attempt
    =/  t  nop-tx
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      %:  link-checks:sal  who  0  link1  gen-tx
          t(is ~[(mk-input gen-id 1 keypath-wit)])
          [0 0]  `0
      ==
...
++  test-link-checks-offset
  ::  prev holds the sat in output 1 at offset 2; the spending tx's outputs
  ::  are [2 5], so landing index 2 falls in output 1 at offset 0
  =/  prev
    %^    mk-tx
        0xaaaa
      ~[(mk-input 0x9 0 ~)]
    ~[[(p2tr-spk 0x1) 3] [(p2tr-spk q1) 10]]
  =/  this
    %^    mk-tx
        0xbbbb
      ~[(mk-input 0xaaaa 1 keypath-wit)]
    ~[[(p2tr-spk 0x2) 2] [(p2tr-spk 0x3) 5]]
  =/  lnk=link:sa  [0xbbbb 0 [ikey1 noop-leaf] ~[noop-rs]]
  ;:  weld
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      (link-checks:sal who 0 lnk prev this [1 2] `0)
    ;:  weld
      (expect !>((levy cs |=(c=check:sa ok.c))))
      %+  expect-eq
    ...
    =/  [cs=(list check:sa) nx=(unit [vout=@ud off=@ud])]
      (link-checks:sal who 0 lnk prev fee-tx [1 2] `0)
    ;:  weld
...
::  ==========================================================================
++  test-check-spawn
  =/  [cs=(list check:sa) psat=(unit sont:ord)]  (check-spawn:sal good-sat pre-tx)
  ;:  weld
    (expect !>((levy cs |=(c=check:sa ok.c))))
    %+  expect-eq
      !>  `(unit sont:ord)``[pre-id 0 0]
      !>  psat
  ==
::
++  test-check-spawn-fig-mismatch
  =/  s  good-sat
  =/  [cs=(list check:sa) psat=(unit sont:ord)]
    (check-spawn:sal s(who ~marzod) pre-tx)
  ;:  weld
...
++  test-run-checks-valid
  ...
  ;:  weld
    ...
    %+  expect-eq
      ...
      !>  tip-value.res
    %+  expect-eq
      !>  `(unit point:urb)``[[[nop-id 0 0] ~] 0 1 pas [%.n who] ~ ~]
      !>  point.res
  ==
::
++  test-run-checks-bad-tip
  =/  s  good-sat
  =/  res  (run-good s(tip [nop-id 0 5]) ~)
  ;:  weld
    (expect !>(!ok.verdict.res))
    (expect !>(!(got-check verdict.res 'tip-sont')))
  ==
::
++  test-run-checks-commit-not-spending
  ::  a commit that does not spend the attested precommit output: the
  ::  shared-controller proof fails (comets are not mintable off other
  ::  people's precommits)
  =/  bad-cmt  (mk-tx cmt-id ~[(mk-input 0x9999 0 ~)] ~[[(p2tr-spk q0) 10.000]])
  =/  res
    (run-checks:sal good-sat ~[gen-tx nop-tx] bad-cmt pre-tx `%.y ~ `0 ~[`0 `0])
  ;:  weld
    (expect !>(!ok.verdict.res))
    (expect !>(!(got-check verdict.res 'spawn-commit-spends-precommit')))
  ==
::
++  test-run-checks-tracked
  ;:  weld
    ::  verifier holds the commit-sat (entering link 0): it is behind -> ok
    (expect !>(ok.verdict:(run-good good-sat `[cmt-id 0 0])))
    ::  verifier holds the tip: up to date -> ok
    (expect !>(ok.verdict:(run-good good-sat `[nop-id 0 0])))
