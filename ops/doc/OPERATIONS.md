...
The opposite of the table above: here the docs were right and the code was
wrong, and the code has since been fixed. Kept so the old behaviour is
recognisable in logs and piers that predate the fix.

| former disagreement | what changed |
|---|---|
| **`tracked-prefix` failing means "a fork, not an old copy"** — `+prefix-chain` could not tell the two apart. It answered `%.n` both when the logs *diverge* (a fork) and when the new log is a **strictly shorter identical prefix** (an old copy), so a comet replaying its own genuine earlier attestation — which a reboot from the un-refreshed boot feed produces (§6) — was classed fraud and snubbed, alongside three stale-class checks in the same verdict that all said "old copy". Reproduced on two verifiers 2026-08-07. | Fixed in `cdaf7cf`. `+prefix-chain` is replaced by `$log-relation` (`%same` / `%extends` / `%behind by=n` / `%fork at=i`), judged by `+anchor-ok`: a **fork at any position** is fraud regardless of length; **behind by any number of entries** is **forgiven** and the verdict comes out `%stale` — demote to a fresh `%alien`, never a snub. (Amended 2026-08-10: it was "behind by exactly one is forgiven, two or more is fraud". An attestation packet is a bearer token, so a third party who kept an old one can replay it, and a replay is always a *prefix* and never a fork — so `%behind` is exactly the class an attacker can reach, and reading its depth as guilt handed any observer a way to get an honest comet permanently snubbed. Interim: it does not close the replay itself.) A new stale-class check **`tracked-lag`** fires on every `%behind`, so a forgiveness is visible in the report rather than silent, and `tracked-prefix` remains the *judgement* passed on it. The comparison now keys on hop **identity** (`+hop-id` / `+spawn-id`, which strip `height` and `start-height`), so an ordinary reorg that re-mines a custody transaction one block over no longer reads as a fork — see the second entry in `doc/live-tests/MATRIX-COMPLETION-RESULTS.md`'s findings. Height survives only as a fetch hint and an ordering bound. |
| **a snub is undone by a later positive verdict** — it was not. `+sy-sybl`'s `%full` branch (`sys/vane/ames.hoon`) installed the point and never touched `ships.snub`; only `%fail` wrote it. Verified live 2026-08-07: a comet re-verified VALID with its snub intact. | Fixed in the kernel, `f68a547b2b`: `+sy-sybl`'s `%full` branch now calls `(sy-snub %deny %del ~[her])`, so a positive verdict lifts an existing snub. Read it as a **safety net, not self-healing**: no transport will carry the attestation that earns the `%full`. Classic ames `+pe-hear` tests `ships.snub` the moment it has a `$shot`, before it classifies the packet at all; mesa's `+pe-heer` gated only its `%poke` branch until `b0a8e962ff` put the same test on `%page` — keyed on `her.name`, the ship `+al-take-proof` would attest — closing the last route by which a snubbed comet could re-attest and be readmitted. **A snub now holds on every transport**, and it does not expire: a per-ship TTL is durable state an attacker can make us allocate, so there is deliberately no decay and no sweep. The only positive verdict that reaches a snubbed ship is therefore one an operator asked for — re-poke the `%jael-writ` (`ops/gwctl.py writ`) and the `%verdict` lifts the snub as a consequence of the judgement. The blunt alternative is a direct blocklist edit, `ops/gwsnub.py` or an ames `%snub %deny %del`, which un-snubs without verifying anything. |
