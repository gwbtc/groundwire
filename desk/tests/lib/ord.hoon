...
+|  %constants
::
::  https://ordinals.com/inscription/52016913
...
++  ord-0-script  (need (de:bscr ord-0-octs))
++  ord-0-mails   (mails:de:lib ord-0-script)
++  ord-0-mail    (head ord-0-mails)
++  ord-0-drafts  (drafts:de:lib ord-0-script)
::
::  XX $urb-octs, etc.
::
::
...
::
++  make-tx
  |=  [is=(list inputw:tx:bitcoin) os=(list @ud)]
  ^-  dataw:tx:bitcoin
  =+  tx=*dataw:tx:bitcoin
  tx(is is, os (turn os |=(@ [2^0xb0b +<])))
::
++  make-ord-insc
  |=  [txh=@ux pos=@ud val=@ud mails=(list mail:lib)]
  (make-input-wit txh pos val ~[(en:bscr (mails-to-script:en:lib mails)) [3 0x1.7355]])
::
++  make-input-wit
  |=  [txh=@ux pos=@ud val=@ud witness=(list octs)]
  ^-  inputw:tx:bitcoin
  =+  in=*inputw:tx:bitcoin
  ::  in(txid 32^txh, pos pos, value val, witness witness)
  in(txid txh, pos pos, value val, witness witness)
--
::
|%
::
+|  %de-core
::
++  test-de-draft-to-mail
  %+  expect-eq
    !>  ord-0-mail
    !>  (draft-to-mail:de:lib (head ord-0-drafts))
::
++  test-de-unv
  ::  XX should this produce ~? when would it not?
  %+  expect-eq
    !>  ~
