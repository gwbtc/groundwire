...
`%gw-btc` is Groundwire's equivalent of `%azimuth` and `%eth-watcher`. It regularly polls Bitcoin for comet "attestations" and updates its ship's Jael state accordingly.
...
2. After mining the comet but before booting it, a `%spawn` attestation is submitted to Bitcoin that includes the comet's public networking key, notifying all Groundwire ships running `%gw-btc`.
...
5. After booting, the new ship's `%gw-btc` agent will take some time to get synced to the chain. (Eventually we'll make this more performant with snapshotting.)

Groundwire's current onboarding script automatically generates a new Bitcoin address, queries a faucet for a funding UTXO (if you've provided an invite code; if not, you need to fund this address yourself), mines a comet, submits the `%spawn` and `%escape` transactions (requesting a signature from the hardcoded sponsor for the `%escape`), waits a few blocks, and then boots the ship.
