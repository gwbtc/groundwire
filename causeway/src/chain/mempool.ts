...
// Default to the Vite dev proxy when running in the browser, falling back to
// direct mempool.space in Node/test contexts.
function defaultMempoolUrl(network: "main" | "testnet" = "main"): string {
  const suffix = network === "testnet" ? "/testnet/api" : "/api";
  if (typeof window !== "undefined" && window.location) {
    return `${window.location.origin}/_proxy/mempool${suffix}`;
  }
  return `https://mempool.space${suffix}`;
}

export class Mempool {
  readonly baseUrl: string;
  constructor(opts: { baseUrl?: string; network?: "main" | "testnet" } = {}) {
    this.baseUrl = opts.baseUrl ?? defaultMempoolUrl(opts.network);
  }
