...
export interface Snapshot {
  ...
  // and a dropped fief silently produces a different output key — an
  // unsignable PSBT and a state-key that no verifier can match. It must
  // therefore be carried through a rekey verbatim, never hardcoded to null.
  fief: Fief | null;
}
...
function unit(v: Noun | null): Noun {
  return v === null ? 0n : [0n, v];
}

// A Hoon list as a null-terminated noun: ~[a b c] = [a [b [c 0]]].
function listToNoun(items: Noun[]): Noun {
  let n: Noun = 0n;
  for (let i = items.length - 1; i >= 0; i--) n = [items[i]!, n];
  return n;
}

// %turf = 'turf' LE, %if = 'if' LE, %is = 'is' LE — the Hoon tag atoms.
export const FIEF_TAG = {
  turf: 0x66727574n,
  if: 0x6669n,
  is: 0x7369n,
} as const;

function atom(v: bigint | number, what: string): bigint {
  const b = typeof v === "bigint" ? v : BigInt(Math.trunc(v));
  if (typeof v === "number" && !Number.isSafeInteger(v)) {
    throw new Error(`fiefToNoun: ${what} is not a safe integer (${v})`);
  }
  if (b < 0n) throw new Error(`fiefToNoun: ${what} is negative (${v})`);
  return b;
}

// $fief as a noun — [tag [p q]], matching sur/urb.hoon and, byte for byte, the
// desktop tool's +fief_noun (causeway/desktop/causeway.py). The desktop tool is
// the authority: it produced the verified on-chain commitments, so this encoder
// is pinned against jams generated by it (tests/fief.spec.ts).
export function fiefToNoun(f: Fief): Noun {
  switch (f.type) {
    case "if":
      return [FIEF_TAG.if, [atom(f.ip, "%if address"), atom(f.port, "%if port")]];
    case "is":
      return [FIEF_TAG.is, [atom(f.ip, "%is address"), atom(f.port, "%is port")]];
    case "turf":
      return [FIEF_TAG.turf, [
        // p = (list turf), turf = (list @t), tld first.
        listToNoun(f.domains.map((t, i) =>
          listToNoun(t.map((seg, j) => atom(seg, `%turf domain ${i} segment ${j}`))))),
        atom(f.port, "%turf port"),
      ]];
    default: {
      const bad: never = f;
      throw new Error(`fiefToNoun: unknown fief ${JSON.stringify(bad)}`);
    }
  }
}
